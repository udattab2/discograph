...
from tui.screens import AddArtistModal

class DiscographApp(App[None]):
    ...
    cur: sqlite3.Cursor
    left_mode: str
    middle_view_mode: str
    selected_parent_id: int | None
    selected_album_id: int | None
    ...
    def refresh_left_list(self) -> None:
        ...
        if self.left_mode == "artists":
            ...
            for r in rows:
                ...
                item = ValueListItem(f"👤 {art.name}", art.id, id=f"art_{art.id}")
            ...
            for r in rows:
                ...
                item = ValueListItem(f"🎸 {gen.name}", gen.id, id=f"gen_{gen.id}")
                left_list.append(item)
    ...
    async def action_clear_search(self) -> None:
        ...
        await self.action_switch_mode("artists")
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if event.input.id == "search-input":
            ...
            for r in rows:
                ...
                item = ValueListItem(f"🎵 {track.name}", track.id, id=f"track_{track.id}")
                left_list.append(item)
    ...
    def action_add_artist(self) -> None:
        ...
        def handle_result(artist_name: str | None) -> None:
            ...
            self.run_worker(worker_task, exit_on_error=False)
    ...
    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        ...
        if isinstance(item, ValueListItem):
            ...
            if event.list_view.id == "left-list":
                self.on_left_highlighted(val)
            elif event.list_view.id == "middle-list":
                self.on_middle_highlighted(val)
        else:
            pass
    ...
    def on_left_highlighted(self, value: int) -> None:
        ...
        if self.left_mode == "artists":
            ...
            for r in rows:
                ...
                item = ValueListItem(f"💿 {alb.name}{year_str}{tracks_str}", alb.id, id=f"alb_{alb.id}")
            ...
            self.cur.execute(
                "SELECT id, name, length, album_id, genre_id, lyrics FROM track WHERE id=?",
                (value, )
            )
            ...
            if track_row:
                ...
                # Fetch metadata
                self.cur.execute("SELECT name, year, trackcount, artist_id FROM album WHERE id=?", (track.album_id,))
                alb_row = self.cur.fetchone()
                album_name = alb_row[0] if alb_row else "Unknown Album"
                artist_id = alb_row[3] if alb_row else None
                
                artist_name = "Unknown Artist"
                if artist_id:
                    self.cur.execute("SELECT name FROM artist WHERE id=?", (artist_id, ))
                    art_row = self.cur.fetchone()
                    artist_name = art_row[0] if art_row else "Unknown Artist"
    ...
    def on_middle_highlighted(self, value: int) -> None:
        ...
        if self.middle_view_mode == "albums":
            ...
            album_name = alb[0] if alb else "Unknown Album"
            year = alb[1] if alb else ""
            ...
            for r in rows:
                song_num = f"{r[0]}. " if r[0] else ""
                preview_text += f"{song_num}{r[1]} ({r[2]})\n"
    ...
    def show_tracks_for_selected_album(self) -> None:
        ...
        if not isinstance(active_item, ValueListItem):
            return
        ...
        alb = self.cur.fetchone()
        album_name = alb[0] if alb else "Album"
        ...
        for r in rows:
            ...
            item = ValueListItem(f"🎵 {song_num}{track.name} ({track.length})", track.id, id=f"track_{track.id}")
            middle_list.append(item)
    ...
    def show_albums_for_selected_parent(self) -> None:
        ...
        if self.selected_album_id:
            # Build a mapping from album ID to index by iterating only album items.
            id_to_index = {}
            album_idx = 0
            for child in middle_list.children:
                if isinstance(child, ValueListItem) and getattr(child, "id", "").startswith("alb_"):
                    id_to_index[child.value] = album_idx
                    album_idx += 1
            if self.selected_album_id in id_to_index:
                middle_list.index = id_to_index[self.selected_album_id]
